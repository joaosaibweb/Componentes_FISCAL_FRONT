...
export default function DatePickerInput({
  ...
  const [dateFormated, setDateFormated] = useState<string>(value?.format('DD/MM/YYYY') || '');

  useEffect(() => {
    if (value) {
      const formated = dateAndTime ? value.format('DD/MM/YYYY HH:mm:ss') : value.format('DD/MM/YYYY');
  ...
  if (maxDate) {
    maxTimes = setHours(
      setMinutes(new Date(), moment(maxDate).hours() === 0 ? 59 : moment(maxDate).minutes()),
    ...
    <Container>
      ...
      {dateAndTime ? (
        <ContainerDatePicker>
          ...
          onChange={(date: Date | null) => onChangeDate}
          locale={ptBR}
          ...
          minTime={
            minDate
              ? setHours(setMinutes(new Date(), moment(minDate).minutes()), moment(minDate).hours())
              : setHours(setMinutes(new Date(), 0), 0)
          }
          maxTime={maxTimes ?? undefined}
          readOnly={readOnly}
          // timeClassName="input-time"
           // {...rest}
          customInput={
            <input
              className={classes}
      ...
      <ContainerDatePicker>
        <DatePicker
          selected={date.toDate()}
          onChange={(date: Date | null) => onChangeDate(moment(date))}
          locale={ptBR}
